export default function WebPage({webCards}){
    return(
        <div className="grid md:grid-cols-3 sm:grid-cols-2 justify-start m-4">
            {webCards.map((card,i)=>(
                <a href={card.link} key={i} target="_blank" rel="noopener noreferrer" className="card relative m-2 border-2 border-amber-500 rounded-lg overflow-hidden transition-transform duration-300"
                aria-label={`View ${card.title} project`} >
                    ...
                    <div className="flex flex-warp m-2 absolute bottom-0 left-0 z-10">
