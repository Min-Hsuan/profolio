        ...
        <>
            <div className="flex flex-wrap justify-center m-4">
                {graphicCards.map((card,i)=>(
                    <div key={i} className="card relative m-2 border-2 border-amber-500 rounded-lg overflow-hidden w-64 h-64 transition-transform duration-300  cursor-pointer" onClick={()=>openLightbox(card)}onKeyDown={(e)=>e.key === 'Enter' && openLightbox(card)} tabIndex={0} role="button" aria-label={`View ${card.title} in lightbox`} >
